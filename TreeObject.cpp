...
void TreeObject::initModels(Vector3f root, Vector3f dims, int depth, Quatf angleFromY) {
	...
	if (depth == 0) {
		...
		levelX->AddBox(_leafColor, root + angleFromY.Rotate(Vector3f(0, dims.x, 0)), Vector3f(dims.y * 2, dims.x * 2, dims.y * 2), angleFromY);
		...
		levelX->AddBox(_trunkColor, root + angleFromY.Rotate(Vector3f(0, dims.y / 2, 0)), dims, angleFromY);
